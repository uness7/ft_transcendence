...
# if the user is not registered he should be redirected to /register page
# else if the user is registered, you must check if the user has a qr code 
...
class QRSetup(View):
    ...
    def post(self, request, user_id):
        ...
        '''
        entries_count = TOTPDevice.objects.all().count();
        if entries_count > 1:
            return JsonResponse({'message': 'Too many entries!'}, status=400);
        '''
        ...
        try:
            ...
            return JsonResponse({'error': 'There is qr code exists'}, status=400);
        '''
...
class VerifyOTPCode(View):
    ...
    # Body: key generated by scanning the auth application
    def post(self, request, user_id):
        try:
            user = User.objects.get(public_id=user_id);
            username = user.username;
...
# TODO 1: Create a disable functionality
